...
const Forgetpass = () => {
    ...
    const hndlsub = async (e) => {
        e.preventDefault()
        const {usrunm, usreml} = vl

        try {
            const res = await axios.post(`${Server}/user/forget`, {usrunm, usreml})
            ...
            else if (dta.statuscode === 220) {
                window.alert(dta.message)
                navig(`/admin/cnfcode/${dta.data._id}`)
            }
            else {
                window.alert(dta.message)
            }
                ...
                <main className="form-signin w-100 m-auto">
                    <form>
                        ...
                        <div className="form-floating mt-5 mb-1">
                            <input type="text" className="form-control" name="usrunm" id="usrunm" placeholder="name@example.com" onChange={hndlinp} />
                            <label htmlFor="usrunm">Username</label>
                        </div>
                        <div className="form-floating mt-1 mb-5">
                            <input type="email" className="form-control" name="usreml" id="usreml" placeholder="name@example.com" onChange={hndlinp} />
                            <label htmlFor="usreml">Registered Email Id</label>
                        </div>

                        <button className="btn btn-primary w-100 py-2" type="submit" onClick={hndlsub}>Send Code</button>
                        <button className="btn btn-outline-warning w-100 py-2 my-2" onClick={(e) => {
                            e.preventDefault()
                            navig('/admin/login')
                        }}>Back</button>
