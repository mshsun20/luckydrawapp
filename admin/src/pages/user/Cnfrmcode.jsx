...
const Cnfrmcode = () => {
    ...
    const hndlsub = async (e) => {
        ...
        try {
            const res = await axios.post(`${Server}/user/cnfcode`, {rndid:id, ucnfcode})
            const dta = await res.data
            // console.log(dta)

            if (dta.statuscode === 220) {
                window.alert(dta.success)
                navig(`/admin/resetpass/${dta.data.user}`)
            }
            else {
                window.alert(dta.error)
                document.querySelector('#ucnfcode').value = ''
                document.querySelector('#ucnfcode').focus()
            }
        } catch (error) {
            console.error(error)
        }
            ...
            <div className='d-flex align-items-center py-0 bg-body-tertiary' style={{flexDirection:'column',height:'100vh'}}>
                ...
                <main className="form-signin w-100 m-auto">
                    <form>
                        ...
                        <h1 className="h3 mb-3 fw-normal">User Confirmation</h1>

                        <div className="form-floating mt-2 mb-3">
                            <input type="text" className="form-control" name="ucnfcode" id="ucnfcode" placeholder="Password" value={vl&&vl.ucnfcode} onChange={hndlinp} />
                            <label htmlFor="ucnfcode">Confirmation Code</label>
                        </div>

                        <button className="btn btn-success w-100 py-2" type="submit" onClick={hndlsub}>Confirm</button>
                        <button className="btn btn-outline-info w-100 py-2 my-2" onClick={(e) => {
                            e.preventDefault()
                            navig('/admin/forget')
                        }}>Resend</button>
