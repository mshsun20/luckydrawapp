...
const Resetpass = () => {
    ...
    const hndlsub = async (e) => {
        e.preventDefault()
        const {usrpass, usrcnfpass} = vl

        try {
            if (usrcnfpass === usrpass) {
                const res = await axios.put(`${Server}/user/pass/change/`+id, {usrpass})
                ...
                document.querySelector('#usrcnfpass').value = ''
                document.querySelector('#usrcnfpass').focus()
            }
        } catch (error) {
            console.error(error)
        }
  ...
  return (
    <>
        <div className="wbpg">
            <div className='d-flex align-items-center py-0 bg-body-tertiary' style={{flexDirection:'column',height:'100vh'}}>
                <Topbar />
                ...
                <main className="form-signin w-100 m-auto">
                    <form>
                        <div className="brndlgo" style={{position:'relative',width:'100%',padding:'2.5rem'}}><img src={smlogo} alt="SM Logo" style={{width:'100%'}} /></div>
                        <h1 className="h3 mb-3 fw-normal">Change Password</h1>

                        <div className="form-floating mt-5 mb-1">
                            <input type="password" className="form-control" name="usrpass" id="usrpass" placeholder="Password" onChange={hndlinp} />
                            <label htmlFor="usrpass">New Password</label>
                        </div>
                        <div className="form-floating mt-1 mb-5">
                            <input type="password" className="form-control" name="usrcnfpass" id="usrcnfpass" placeholder="Password" onChange={hndlinp} />
                            <label htmlFor="usrcnfpass">Retype Password</label>
                        </div>

                        <button className="btn btn-primary w-100 py-2" type="submit" onClick={hndlsub}>Update</button>
                        <button className="btn btn-outline-danger w-100 py-2 my-2" onClick={(e) => {
                            e.preventDefault()
                            navig('/admin/login')
                        }}>Cancel</button>
