...
const Login = () => {
    ...
    const hndlsub = async (e) => {
        e.preventDefault()
        const {usr_ky, usr_pass} = vl

        try {
            const res = await axios.post(`${Server}/user/login`, {usr_ky, usr_pass})
            const dta = await res.data
            // console.log(dta)
            if (dta.statuscode === 220) {
                window.alert(dta.success)
                localStorage.setItem('user', dta.data)
                navig('/admin')
            }
            else {
                ...
                <main className="form-signin w-100 m-auto">
                    <form>
                        ...
                        <div className="form-floating mt-5">
                            <input type="text" className="form-control" name="usr_ky" id="usr_ky" placeholder="name@example.com" onChange={hndlinp} />
                            <label htmlFor="usr_ky">Employee Code</label>
                        </div>
                        <div className="form-floating mt-2 mb-3">
                            <input type="password" className="form-control" name="usr_pass" id="usr_pass" placeholder="Password" onChange={hndlinp} />
                            <label htmlFor="usr_pass">Password</label>
                        </div>
                        <div className="d-flex justify-content-between form-check text-start mb-5">
                            ...
                            <NavLink className='link-offset-3-hover' to='/admin/forget'>Forget Password</NavLink>
                        </div>
