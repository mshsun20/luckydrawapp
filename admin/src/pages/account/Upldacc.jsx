...
const Upldacc = () => {
    ...
    const [exclFili, setExclFili] = useState(null)
    const [exclFilub, setExclFilub] = useState(null)
    const [excErri, setExcErri] = useState(null)
    const [excErrub, setExcErrub] = useState(null)
    const [exclDatai, setExclDatai] = useState(null)
    const [exclDataub, setExclDataub] = useState(null)
    const [exclMpi, setExclMpi] = useState(null)
        ...
        if (selectedfile) {
            //  console.log(selectedfile.type)
             if (selectedfile&&fileType.includes(selectedfile.type)) {
                ...
                reader.onload = (e) => {
                    setExcErri(null)
                    setExclFili(e.target.result)
                }
             }
             else {
                setExcErri(`Please Select Only Excel File Type: ".csv/.xls/.xlsx"`)
                setExclFili(null)
             }
        }
        else {
    ...
    const hndlsubi = async (e) => {
        e.preventDefault()
        if (exclFili!==null) {
            const workbook = XLSX.read(exclFili, {type: 'buffer'})
    ...
    const hndlupldi = async (e) => {
        e.preventDefault()

        if (window.confirm(`Confirm data Bulk Import...?`)) {
            const res = await axios.post(`${Server}/accounts/upload`, {exclDatai, added_by:usr&&usr[0]._id})
            const stat = await res.data
            // console.log(stat)
            setStts(stat)
            if (stat.statuscode === 220) {
                console.log(stat.message)
                window.alert(stat.success)
    ...
    const downsubi = async (e) => {
        e.preventDefault()

        const wb = XLSX.utils.book_new(),
        ws = XLSX.utils.json_to_sheet(frmti)

        XLSX.utils.book_append_sheet(wb, ws, 'Account Upload Format')
        XLSX.writeFile(wb, 'Acc_format.xlsx')
    }
